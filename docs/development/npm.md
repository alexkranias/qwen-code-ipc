...
This monorepo contains two main packages: `qwen-code-ipc-cli` and `@qwen-code/qwen-code-core`.
...
```bash
npm install -g qwen-code-ipc@nightly
```
...
- `npx -y qwen-code-ipc@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y qwen-code-ipc@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall qwen-code-ipc && npm uninstall -g qwen-code-ipc && npm cache clean --force &&  npm install qwen-code-ipc@<version>`
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace qwen-code-ipc-cli`.
